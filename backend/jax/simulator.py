from jax.numpy import array, kron, log2, trace
from jax.numpy import transpose, conj, identity, real
...
def depolarize(rho, beta):
    return (array(1, int) - beta) * rho + beta / array(rho.shape[0], int) * identity(rho.shape[0])
...
def operateVector(state, gate):
    ...
    permuteLength = array(newStateLen / operatorLength, int)
